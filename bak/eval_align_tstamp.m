...
% data_slam = load('.\data\V103_FT_VI.txt');
data_slam0 = load('.\data\V103_FT_VI.txt');
data_inav0 = load('.\data\V103_FT_IMU.txt');
...
figure;
subplot(3,1,1)
plot(data_slam(:,1), data_slam(:, 2) - data_inav(:,2), 'b')
title('x diff')
subplot(3,1,2)
plot(data_slam(:,1), data_slam(:, 3) - data_inav(:,3), 'g')
title('y diff')
subplot(3,1,3)
plot(data_slam(:,1), data_slam(:, 3) - data_inav(:,3), 'r')
title('z diff')
%
%% attitude comparing
qdiff_slam_inav = quatmultiply(data_slam(:, 5:8), quatconj(data_inav(:, 5:8)));
[anz_dsinav, any_dsinav, anx_dsinav] = quat2angle( qdiff_slam_inav);
figure;
subplot(3,1,1)
plot(data_slam(:,1), anx_dsinav, 'b')
title('anx diff')
subplot(3,1,2)
plot(data_slam(:,1), any_dsinav, 'g')
title('any diff')
subplot(3,1,3)
plot(data_slam(:,1), anz_dsinav, 'r')
title('anz diff')
